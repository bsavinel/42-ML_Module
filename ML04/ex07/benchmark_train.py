...
if __name__ == "__main__":
	...
	Xtrain, Xeval, Ytrain, Yeval = data_spliter(X, Y, 0.8)
	...
	model_loss = {}
	lambdas = [0.0, 0.2, 0.4, 0.6, 0.8, 1]

	tab = []

	for i in range(0,4):
		a=add_polynomial_features(X, i + 1)
		tmpXtrain = normalizer(add_polynomial_features(Xtrain, i + 1), a)
		tmpXeval =  normalizer(add_polynomial_features(Xeval, i + 1),a)
		theta =  np.ones((Xtrain.shape[1] * (i + 1) + 1, 1))

		for j in lambdas:
			linearModel = MyRidge(theta, alpha=0.01, max_iter=300)
