...
#?##############################################?#
	...
	def __add__(self, other):
		if (isinstance(other, Matrix)):
			...
			return type([[self.data[j][i] + other.data[j][i] for i in range(self.shape[1])] for j in range(self.shape[0])])
		else:
	...
	def __radd__(self, other):
		if (isinstance(other, Matrix)):
			if (self.shape != other.shape):
				raise MatrixException("Matrix can be added only if they have the same shape")
			return Matrix([[other.data[j][i] + self.data[j][i] for i in range(self.shape[1])] for j in range(self.shape[0])])
		else:
			raise MatrixException("Matrix can be added only with another matrix")
...
#?##################################################?#
	...
	def __rsub__(self, other):
		if (isinstance(other, Matrix)):
			if (self.shape != other.shape):
				raise MatrixException("Matrix can be sub only if they have the same shape")
			return type([[other.data[j][i] - self.data[j][i] for i in range(self.shape[1])] for j in range(self.shape[0])])
		else:
			raise MatrixException("Matrix can be sub only with another matrix")
...
#?###############################################?#
	...
	def __truediv__(self, scalar):
		if (isinstance(scalar, int) or isinstance(scalar, float)):
			return type([[self.data[j][i] / scalar for i in range(self.shape[1])] for j in range(self.shape[0])])
		else:
...
#?#####################################################?#
	...
	def __mul__(self, other):
		if (isinstance(other, Matrix)):
			...
			return type([sum(self.data[j][i] * other.data[i][k] for i in range(self.shape[1])) for j in range(self.shape[0])] for k in range(other.shape[1]))
